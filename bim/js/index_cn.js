...
$.initPhotoSwipeFromDOM = function(gallerySelector,medDom) {
    ...
    var openPhotoSwipe = function(index, galleryElement, disableAnimation, fromURL) {
                    ...
                    +'<div class="pswp__scroll-wrap">'
                        ...
                        +'<div class="pswp__ui pswp__ui--hidden">'
                            +'<div class="pswp__top-bar">'
                                ...
                                +'<button class="pswp__button pswp__button--close" title="閫�鍑虹敾寤�"></button>'
                                +'<button class="pswp__button pswp__button--fs" title="Toggle fullscreen"></button>'
                                +'<button class="pswp__button pswp__button--zoom" title="鏀惧ぇ / 缂╁皬"></button>'
                                // +'<button class="pswp__button pswp__button--rotate-left" title="閫嗘椂閽堟棆杞�"><i class="icon md-rotate-ccw" aria-hidden="true"></i></button>'
                                // +'<button class="pswp__button pswp__button--rotate-right" title="椤烘椂閽堟棆杞�"><i class="icon md-rotate-cw" aria-hidden="true"></i></button>'
        ...
        options = {
            ...
            // captionEl:false,
            shareEl:false,
            errorMsg:'<div class="pswp__error-msg"><a href="%url%" target="_blank">鍥剧墖</a> 鍔犺浇澶辫触</div>'
        };
        if (fromURL) {
 ...
 */
$(function(){
	// 缂栬緫鍣ㄥ搷搴斿紡琛ㄦ牸锛堥渶璋冪敤tablesaw鎻掍欢锛�
			...
			if($("img",this).length && !$(this).hasClass('no-gallery')){
				...
				$("img",this).one('click',function(){
	    			...
	    			if(editorimg_gallery_open){
		    			$.initPhotoSwipeFromDOM('.met-editor');//锛堥渶璋冪敤PhotoSwipe鎻掍欢锛�
...
METUI_FUN['head_nav_met_16_4'] = {
    ...
    cntotc:function(){
            //绠�浣撶箒浣撲簰鎹�
            var b=METUI['head_nav_met_16_4'].find('.btn-cntotc');
...
METUI_FUN['foot_info_met_11_1'] = {
    ...
    cntotc:function(){
            //绠�浣撶箒浣撲簰鎹�
            var b=METUI['foot_info_met_11_1'].find('.btn-cntotc');
    ...
    bg: function() {
            $('.foot_info_met_11_1_bottom[data-bg]').each(function(index, el) {
            var background = $(this).attr('data-bg'),
                ...
                ifbotc = $('.foot_info_met_11_1_bottom').data('ifbotc');
                if(ifbotc){
    ...
    padding:function(){
        if(METUI['foot_info_met_11_1'].length){
            var h_m=$('.foot_info_met_11_1_bottom').height();
            var p_d= parseInt(METUI['foot_info_met_11_1'].css("padding-bottom").replace('px', ''))+h_m;
            $(window).resize(function(){
                pd();
            })
            function pd(){
                if($('.foot_info_met_11_1_bottom').hasClass('iskeshi') || $(window).width()<768){
                        METUI['foot_info_met_11_1'].css("padding-bottom",p_d);
...
var x = new metui(METUI_FUN['foot_info_met_11_1']);

METUI_FUN['product_list_met_11_1'] = {
    name: 'product_list_met_11_1',
    init: function() {
        var list = METUI['product_list_met_11_1'].find('.product-list');
        if (list.length) {
            var rz = list.attr('data-resize').split('|'),
                num = list.data('num'),
                prev = METUI['product_list_met_11_1'].find('.control-prev'),
                next = METUI['product_list_met_11_1'].find('.control-next');
            if (rz[0] == '') {
                return;
            }
            list.slick({
    ...
    wap: function() {
        var ul = METUI['product_list_met_11_1'].find('.nav-tabs');
        Breakpoints.on('xs', {
            enter: function() {
                ul.navtabSwiper();
            }
    ...
    appear: function() {
        if (METUI['product_list_met_11_1'].length) {
            var repeat = false;
            function animation() {
                if (repeat == false) {
                    var title = METUI['product_list_met_11_1'].find('.title'),
                        subtitle = METUI['product_list_met_11_1'].find('.subtitle'),
                        desc = METUI['product_list_met_11_1'].find('.desc'),
                    ...
                    var item = METUI['product_list_met_11_1'].find('.item');
                    var navitem = METUI['product_list_met_11_1'].find('.nav-link');
            ...
            var scene = new ScrollMagic.Scene({
                triggerElement: '.product_list_met_11_1',
                offset: -300
            }).addTo(controller);
    ...
    navtab:function(){
        // 浜у搧鍒楄〃
        var $met_indexpro_navtabs=METUI['product_list_met_11_1'].find(".nav-tabs");
        var list=METUI['product_list_met_11_1'].find('.index-product-list');
        var list_li=$met_indexpro_navtabs.find('li');
        if($met_indexpro_navtabs.length){
            $met_indexpro_navtabs.navtabSwiper();// 閫夐」鍗℃按骞虫粴鍔�
        }
    },
};
var x = new metui(METUI_FUN['product_list_met_11_1']);

METUI_FUN['about_list_met_11_1'] = {
    name: 'about_list_met_11_1',
    appear: function() {
        if (METUI['about_list_met_11_1'].length) {
            var repeat = false;
            function animation() {
                if (repeat == false) {
                    var title = METUI['about_list_met_11_1'].find('.title'),
                        subtitle = METUI['about_list_met_11_1'].find('.subtitle'),
                        desc = METUI['about_list_met_11_1'].find('.desc'),
                        editor = METUI['about_list_met_11_1'].find('.text'),
                        img = METUI['about_list_met_11_1'].find('.about-img'),
                        a = TweenMax.to(title, 0.6, {
                            autoAlpha: 1,
                            x: 0,
                            ease: Power3.ease,
                        ...
                        d = TweenMax.to(editor, 0.6, {
                            autoAlpha: 1,
                            x: 0,
                            ease: Power3.easeInOut,
                        }),
                        e = TweenMax.to(img, 0.6, {
                            autoAlpha: 1,
                            x: 0,
                            ease: Power3.ease,
                        });
            ...
            var controller = new ScrollMagic.Controller();
            var scene = new ScrollMagic.Scene({
                triggerElement: '.about_list_met_11_1',
                offset: -300
            }).addTo(controller);
            scene.on("enter", function(event) {
                animation();
      ...
      init: function() {
        var a=METUI['about_list_met_11_1'].find(".video-content span");
        a.click(function(){
            $(this).fadeOut();
            METUI['about_list_met_11_1'].find('video').trigger('play').attr('controls','controls'); 
        }); 
    }
};
var x = new metui(METUI_FUN['about_list_met_11_1']);

METUI_FUN['news_list_met_11_1'] = {
    name: 'news_list_met_11_1',
    appear: function() {
        if (METUI['news_list_met_11_1'].length) {
            var repeat = false;

            function animation() {
                if (repeat == false) {
                    var title = METUI['news_list_met_11_1'].find('.title'),
                        subtitle = METUI['news_list_met_11_1'].find('.subtitle'),
                        desc = METUI['news_list_met_11_1'].find('.desc'),
                        a = TweenMax.to(title, 0.6, {
                            autoAlpha: 1,
                            y: 0,
                            ease: Power3.ease,
                    ...
                    var headAnimate = new TimelineMax();
                    headAnimate.add(a).add(c);
                    var media = METUI['news_list_met_11_1'].find('.media');
                    var i = TweenMax.staggerTo(media, 0.6, {
                        autoAlpha: 1,
                        x: 0,
                        ease: Power3.ease,
                    }, 0.1);
            ...
            var controller = new ScrollMagic.Controller();
            var scene = new ScrollMagic.Scene({
                triggerElement: '.news_list_met_11_1',
                offset: -300
            }).addTo(controller);
            scene.on("enter", function(event) {
                animation();
...
var x = new metui(METUI_FUN['news_list_met_11_1']);
